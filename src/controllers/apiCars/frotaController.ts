...
import { BaseDatabase } from "../../models/BaseDatabase";
  ...
  try {
    ...
    if (searchTerm === undefined) {
      ...
      const result = await db.raw(
        `SELECT * FROM products WHERE description LIKE "Light" OR description LIKE "Hatch" OR description LIKE "Sedan" OR description LIKE "Prime" OR description LIKE "Lux"`
      );
...
export const editProductById = async (req: Request, res: Response) => {
  try {
    ...
    if (nameSelect !== undefined) {
      const confereNome = await db.raw(
    ...
    const [product4edit] = await db.raw(
      `SELECT * FROM products WHERE id="${id}"`
    );
    if (product4edit) {
      (product4edit.id = id),
        (product4edit.name = nameSelect || product4edit.name),
        (product4edit.description = matchDescriptionCategory(
          product4edit.price
        )),
