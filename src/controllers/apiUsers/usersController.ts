...
import { TUserDB, TUser } from "../../types/types";import { User } from "../../models/User";
...
export const getUsers = async (req: Request, res: Response) => {
  try {
   ...
   throw new Error ('"404" usuario nao cadastrado');
    ...
    const users: User[] | undefined= usersDB.map((userDB:TUserDB| undefined) => new User(
          userDB.id,
          userDB.name,
          userDB.nickname.toLowerCase(),
          userDB.password,
          userDB.email.trim().toLowerCase(),
          userDB.created_at,
          userDB.avatar_img,
          userDB.role
        ));
