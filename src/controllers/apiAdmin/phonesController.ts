...
export const getAllPhones = async (req: Request, res: Response) => {
  try {
    ...
    if (!q) {
      ...
      const phones: Phone[] = phonesDB.map(
        (phoneDB) =>
          new Phone(phoneDB.id, phoneDB.user_id, phoneDB.phone_number)
      );
...
export const getPhonesById = async (req: Request, res: Response) => {
  try {
    ...
    const [phoneDB] = await db.raw(
      `SELECT * FROM phones WHERE id='${idSearched}'`
    );
...
export const createPhone = async (req: Request, res: Response) => {
  try {
    ...
    if (createParamsId !== undefined) {
      res.status(400);
      throw new Error(
        "'400': O ID para telefones deve ser gerado automaticamente"
      );
    } else {
