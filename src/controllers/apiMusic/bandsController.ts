...
export const createBand = async (req: Request, res: Response) => {
  try {
    ...
    const [nameDBBands]: string | undefined = await db.raw(
      `SELECT name FROM bands WHERE name like "${bandName}"`
    );
    ...
    const [nameDBUsers]: string | undefined = await db.raw(
      `SELECT name FROM users WHERE name like "${bandName}"`
    );
...
export const editBand = async (req: Request, res: Response) => {
  try {
    ...
    const [band4edit] = await db.raw(
      `SELECT * FROM bands WHERE id="${bandId}"`
    );
    if (band4edit) {
      (band4edit.id = bandId), (band4edit.name = bandName || band4edit.name);
    }
    await db("bands")
      .update(band4edit)
      .where({ id: `${bandId}` });

    const [UBand4edit] = await db.raw(
      `SELECT * FROM users WHERE id="${bandId}"`
    );
    if (UBand4edit) {
      (UBand4edit.id = bandId), (UBand4edit.name = bandName || band4edit.name);
